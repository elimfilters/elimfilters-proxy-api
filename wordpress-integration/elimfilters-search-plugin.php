...
class ELIMFILTERS_Search {
    ...
    public function handle_ajax_generate_pdf() {
        ...
        $dutyText = ($canonical['duty'] === 'HD') ? 'Heavy Duty' : (($canonical['duty'] === 'LD') ? 'Light Duty' : ($canonical['duty'] ?: 'N/A'));
        $oemLine = (count($canonical['oem_codes']) > 0) ? implode(', ', $canonical['oem_codes']) : 'N/A';
        ob_start();
        ?>
        ...
        h2 { color: #FFF12D; margin-top: 20px; font-size: 16px; }
        .divider { border-top: 2px solid #FFF12D; margin: 10px 0 20px; }
        ...
        .footer { text-align: center; color: #7f8c8d; font-size: 10px; margin-top: 30px; }
          ...
          <div class="row"><span class="label">Application Type:</span><span><?php echo esc_html($dutyText); ?></span></div>
          <div class="row"><span class="label">Manufacturer:</span><span><?php echo esc_html($canonical['source'] ?: 'N/A'); ?></span></div>
          <div class="row"><span class="label">OEM Number:</span><span><?php echo esc_html($oemLine); ?></span></div>
          <h2>Description</h2>
